...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and ("Drums" in str(filepath)) and ("Hip Hop" in str(filepath)): 
            contentbeats.append(filepath)

        if  filepath.endswith(".wav") and (("Pad" in str(filepath)) and ("World" in str(filepath))) :
            contentdrones.append(filepath)

        if  filepath.endswith(".wav") and ("Hip Hop" in str(filepath)) and ("Bass" in str(filepath)):
            contentbass.append(filepath)

        if  filepath.endswith(".wav") and ("Rhodes" in str(filepath)) and ("Space" in str(filepath)):
            contentorg.append(filepath)

        if  filepath.endswith(".wav") and ("Piano" in str(filepath)) and ("One Shot" in str(filepath)) :
            contentsax.append(filepath)

        if  filepath.endswith(".wav") and ("Torn" in str(filepath)) and ("Soft" in str(filepath)) :
            contentgit.append(filepath)

        if  filepath.endswith(".wav") and ("Twinkle" in str(filepath)) and ("Amplitude" not in str(filepath)) :
